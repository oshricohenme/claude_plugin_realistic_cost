...
function sectionSubtotal(label: string, hours: number, cost: number, opts?: { bold?: boolean; muted?: boolean }): string {
  return receiptLine(label, hours.toFixed(1) + "h", "", money(cost), opts)
}
...
export function renderTerminal(report: CostReport): string {
  ...
  // Column header
  L.push(`  ${kleur.gray(pad("LINE ITEM", 36))} ${kleur.gray(pad("QTY", 7, "right"))} ${kleur.gray(pad("RATE", 9, "right"))} ${kleur.gray(pad("AMOUNT", 12, "right"))}`)
  ...
  L.push(`  ${kleur.gray("─".repeat(W - 2))}`)
  L.push(sectionSubtotal("Management Overhead Subtotal", mgmtHours, mgmtCost, { bold: true }))
  L.push("")
  ...
  L.push(`  ${kleur.gray("─".repeat(W - 2))}`)
  L.push(sectionSubtotal("Value Creation Subtotal", valueHours, valueCost, { bold: true }))
  L.push("")
  ...
  for (const a of coord) {
    const rate = a.hours > 0 ? a.cost / a.hours : 0
    L.push(receiptLine(a.activity, a.hours.toFixed(1) + "h", rateStr(rate), money(a.cost), { indent: true, muted: true }))
  }
  L.push(`  ${kleur.gray("─".repeat(W - 2))}`)
  L.push(sectionSubtotal("Coordination Tax Total", coordHours, coordCost, { bold: true, muted: true }))
  L.push(`  ${kleur.gray(`  (${coordPct.toFixed(0)}% of grand total is coordination overhead)`)}`)
  L.push("")

  // Grand total
  L.push(kleur.cyan("═".repeat(W)))
  L.push(`  ${kleur.bold(pad("GRAND TOTAL", 36))} ${kleur.bold(pad(report.totalHours.toFixed(1) + "h", 7, "right"))} ${kleur.gray(pad("", 9, "right"))} ${kleur.bold().cyan(pad(money(report.totalCost), 12, "right"))}`)
  L.push(kleur.cyan("═".repeat(W)))
  L.push("")

  // Footer
  L.push(`  ${kleur.gray("Effort:")} ${kleur.bold().yellow(`${cal.manDays.toFixed(1)} man-days`)} ${kleur.gray("·")} ${cal.humanReadable}`)
  if (report.aiCost > 0) {
    L.push(`  ${kleur.gray("AI cost:")} ${kleur.green(`$${report.aiCost.toFixed(2)}`)}`)
  }
  L.push("")

  // Metadata
  const s = report.stats
  L.push(`  ${kleur.gray("Lines:")} +${report.aiLinesAdded} / -${report.aiLinesRemoved}  ${kleur.gray("Tools:")} ${s.toolCalls.total}  ${kleur.gray("Thinking:")} ${s.thinkingTurns}`)
